...
async function main() {
  ...
  console.log(
    `options: preview=${preview}, apply=${apply}, limit=${limit ?? "none"}, restaurant=${targetRestaurant ?? "any"}`,
  );

  // load menu->restaurant mappings
  const links = await prisma.menuItemOnRestaurant.findMany({
    select: { menuItemId: true, restaurantId: true },
  });
  ...
  if (menuIds.length === 0) {
    console.log(
      "No menu->restaurant mappings found (table menu_item_on_restaurant is empty). Nothing to do.",
    );
    return;
  }
  ...
  const itemsRaw = await prisma.orderItem.findMany({
    where,
    select: { id: true, orderId: true, menuItemId: true },
    take: limit,
  });

  // filter to those whose menuItemId has a mapping
  const candidates = itemsRaw.filter(
    (it) => map[it.menuItemId] && map[it.menuItemId].length > 0,
  );
  // optionally filter to a specific restaurant
  const toProcess = targetRestaurant
    ? candidates.filter((it) => map[it.menuItemId].includes(targetRestaurant))
    : candidates;
  ...
  console.table(
    toProcess.map((it) => ({
      ...
      mapped: map[it.menuItemId].join(","),
    })),
  );

  if (preview && !apply) {
    console.log(
      "Preview mode. No changes were made. Rerun with --apply to perform updates.",
    );
    return;
  }
  ...
  if (!apply) {
    console.log(
      "No --apply flag provided. Use --apply to actually update rows. Exiting.",
    );
    return;
  }
  ...
  for (const it of toProcess) {
    ...
    const assign =
      targetRestaurant && restaurants.includes(targetRestaurant)
        ? targetRestaurant
        : restaurants[0];
    try {
      await prisma.orderItem.update({
        where: { id: it.id },
        data: { restaurantId: assign },
      });
      updated++;
    } catch (err) {
      console.error(
        `Failed to update order_item ${it.id}:`,
