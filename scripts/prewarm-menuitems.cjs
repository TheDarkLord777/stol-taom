...
(async function main() {
    ...
    try {
        ...
        const rows = await prisma.menuItem.findMany({ orderBy: { name: 'asc' }, select: { id: true, name: true, slug: true, logoUrl: true, createdAt: true } });
        const items = rows.map(m => ({ id: m.id, name: m.name, slug: m.slug, logoUrl: m.logoUrl ?? undefined, createdAt: m.createdAt.getTime() }));
        ...
        for (const limit of limits) {
            ...
            console.log('[prewarm] wrote', redisKey, 'count=', Math.min(limit, items.length));
        }
