...
async function main() {
    const enabled = String(process.env.ENABLE_REDIS || '').toLowerCase() === 'true' || process.env.ENABLE_REDIS === '1';
    if (!enabled) {
        console.error('Redis not enabled (ENABLE_REDIS not set). Cannot bump menu cache.');
        process.exit(2);
    }
