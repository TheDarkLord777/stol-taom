...
if (!fs.existsSync(depsPath)) {
  const starter = {
    __meta:
      "Edit `dependencies` and `devDependencies` inside this file, then run `npm run sync-deps` to update package.json",
  ...
  console.log(
    "Edit deps.dependencies.json to the desired dependency set, then run this script again to sync.",
  );
  process.exit(0);
}
...
function diffDeps(oldDeps = {}, newDeps = {}) {
  ...
  for (const k of allKeys) {
    ...
    if (o === undefined && n !== undefined)
      added.push({ package: k, version: n });
    else if (o !== undefined && n === undefined)
      removed.push({ package: k, version: o });
...
function printDiffSection(title, d) {
  if (d.added.length || d.removed.length || d.changed.length) {
    ...
    if (d.added.length) {
      console.log("  Added:");
      d.added.forEach((i) => {
        console.log("   +", i.package, i.version);
      });
    }
    if (d.removed.length) {
      console.log("  Removed:");
    ...
    if (d.changed.length) {
      console.log("  Changed:");
      d.changed.forEach((i) => {
        console.log("   *", i.package, i.from, "->", i.to);
      });
    }
  } else {
    console.log(`\n${title}: (no changes)`);
