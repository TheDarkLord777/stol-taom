...
function scan() {
  ...
  if (fs.existsSync(nextCfgPath)) {
    ...
    const hasHeadersFn =
      /\bheaders\s*:\s*async\s*\(\)\s*=>|\basync\s*headers\s*\(/.test(cfg) ||
      /export\s+async\s+function\s+headers\s*\(/.test(cfg);
    ...
    if (!hasHeadersFn) {
      addIssue(
        issues,
        "high",
        "Security headers not configured in next.config.ts (headers()). Add CSP, XFO, Referrer-Policy, Permissions-Policy, X-Content-Type-Options.",
        nextCfgPath,
      );
    } else {
      for (const h of requiredHeaders) {
        if (!cfg.includes(h)) {
          addIssue(
            issues,
            "high",
            `Header missing in next.config.ts: ${h}`,
            nextCfgPath,
          );
        }
      }
    ...
    addIssue(
      issues,
      "high",
      "next.config.ts not found; cannot verify security headers.",
      null,
    );
  }

  // 2) CSRF protection for mutating routes: look for any mention of csrf or origin checks
  const hasCsrfMention = Array.from(byPath.values()).some((v) =>
    /csrf|x-csrf-token/i.test(v),
  );
  const middlewarePath = path.join(ROOT, "middleware.ts");
  const middlewareContent = fs.existsSync(middlewarePath)
    ? safeRead(middlewarePath)
    : "";
  const checksOriginOrReferer =
    /(request\.headers\.get\(['"](origin|referer)['"]\))/i.test(
      middlewareContent,
    ) || /new\s+URL\(/.test(middlewareContent);
  if (!hasCsrfMention && !checksOriginOrReferer) {
    addIssue(
      issues,
      "high",
      "No CSRF token or origin/referer checks detected for mutating requests. Consider adding CSRF protection in middleware for POST/PUT/PATCH/DELETE.",
      "middleware.ts",
    );
  }
  ...
  if (jwtContent) {
    if (!/httpOnly\s*:\s*true/.test(jwtContent))
      addIssue(issues, "high", "JWT cookies should be httpOnly: true", jwtPath);
    const secureOk =
      /secure\s*:\s*true/.test(jwtContent) ||
      /secure\s*:\s*process\.env\.NODE_ENV\s*===\s*["']production["']/.test(
        jwtContent,
      );
    if (!secureOk)
      addIssue(
        issues,
        "high",
        'JWT cookies should be secure: true (or gated by NODE_ENV === "production")',
        jwtPath,
      );
    if (!/sameSite\s*:\s*['"](lax|strict)['"]/i.test(jwtContent))
      addIssue(
        issues,
        "high",
        'JWT cookies should set sameSite: "lax" or "strict"',
        jwtPath,
      );
  } else {
    ...
    for (const [file, content] of byPath.entries()) {
      ...
      if (!/httpOnly\s*:\s*true/.test(content))
        addIssue(issues, "high", "Cookie set without httpOnly: true", file);
      if (!/secure\s*:\s*true/.test(content))
        addIssue(issues, "high", "Cookie set without secure: true", file);
      if (!/sameSite\s*:\s*['"](lax|strict)['"]/i.test(content))
        addIssue(
          issues,
          "high",
          'Cookie set without sameSite: "lax" or "strict"',
          file,
        );
    }
  }
  ...
  let pkg = {};
  try {
    pkg = JSON.parse(safeRead(pkgJsonPath) || "{}");
  } catch {}
  const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
  const hasRateLimitDep = Object.keys(deps).some((d) =>
    /(rate[- ]?limit|upstash|lru|bottleneck)/i.test(d),
  );
  const rateInCode = Array.from(byPath.values()).some((v) =>
    /rate\s*limit|ratelimit/i.test(v),
  );
  if (!hasRateLimitDep && !rateInCode) {
    addIssue(
      issues,
      "medium",
      "No rate limiting detected. Add a simple limiter for login and other sensitive endpoints.",
      "package.json",
    );
  }
  ...
  for (const [file, content] of byPath.entries()) {
    ...
    for (const m of dsi)
      addIssue(
        issues,
        "high",
        "dangerouslySetInnerHTML usage detected. Ensure content is sanitized.",
        file,
        m.idx,
      );
    ...
    for (const m of innerHTML)
      addIssue(
        issues,
        "high",
        "innerHTML assignment detected. Avoid or sanitize input.",
        file,
        m.idx,
      );
    ...
    for (const m of newFn)
      addIssue(
        issues,
        "high",
        "new Function() detected. Avoid dynamic code execution.",
        file,
        m.idx,
      );
  }
  ...
  for (const [file, content] of byPath.entries()) {
    ...
    for (const m of corsAny)
      addIssue(
        issues,
        "high",
        "CORS wildcard (*) detected. Restrict origins in production.",
        file,
        m.idx,
      );
  }
  ...
  // 7) Swagger gating
  const hasSwagger = Array.from(byPath.values()).some((v) =>
    /next-swagger-doc|swagger-ui-react/i.test(v),
  );
  const hasDocsEnvGate = Array.from(byPath.values()).some((v) =>
    /ENABLE_API_DOCS/i.test(v),
  );
  if (hasSwagger && !hasDocsEnvGate) {
    addIssue(
      issues,
      "medium",
      "Swagger detected but not gated by ENABLE_API_DOCS.",
      null,
    );
  }

  // 8) JWT_SECRET presence
  const hasJwtSecret = Array.from(byPath.values()).some((v) =>
    /JWT_SECRET/.test(v),
  );
  if (!hasJwtSecret)
    addIssue(
      issues,
      "info",
      "JWT_SECRET not referenced in code. Ensure tokens use a strong secret from env.",
      null,
    );
