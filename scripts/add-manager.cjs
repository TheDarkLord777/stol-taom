...
async function main() {
    ...
    await prisma.restaurantManager.create({ data: { restaurantId, userId: user.id } });
    console.log('RestaurantManager mapping created for user', user.id, '->', restaurantId);
    if (tempPassword) console.log('Temp password (for new user):', tempPassword);
    console.log('User can now sign in and access the management page once logged in.');
}
