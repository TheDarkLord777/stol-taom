...
const detailCache = new Map<
  string,
  { value: MenuItemDetail | null; ts: number }
>();
    ...
    try {
      ...
      if (r) {
        try {
          ...
          logger.info("[menuDetailRepo:cache] cached to redis", {
            id,
            key: redisKey,
          });
        } catch (err) {
  ...
  async invalidateCache(id: string): Promise<void> {
    try {
      ...
      if (r) {
        ...
        logger.info("[menuDetailRepo:cache] invalidated redis", {
          id,
          key: redisKey,
        });
      }
      ...
      if (r) {
        try {
          ...
          if (keys.length > 0) {
            ...
            await delKeys(keys);
            logger.info("[menuDetailRepo:cache] invalidated redis keys", {
              count: keys.length,
            });
          }
        } catch (err) {
