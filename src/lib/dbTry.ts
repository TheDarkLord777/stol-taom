...
function isTransientPrismaError(e: unknown): boolean {
  ...
  if (/connection.*closed|closed.*connection|ECONNRESET|ETIMEDOUT|Connection terminated/i.test(msg)) return true;
  return false;
}

export async function dbTry<T>(fn: (p: PrismaClient) => Promise<T>): Promise<T> {
  try {
