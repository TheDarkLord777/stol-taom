...
export function Combobox(props: ComboboxProps) {
  ...
  if (props.mode !== 'input') {
    const { placeholder = 'Select…', buttonClassName } = props as ButtonModeProps
    return (
      <Popover.Root open={open} onOpenChange={setOpen}>
        ...
        <Popover.Content sideOffset={8} className="z-50 rounded-md border border-gray-200 bg-white p-0 shadow-lg">
          <Command label="combobox" className="w-64">
            ...
            <Command.List className="max-h-60 overflow-auto py-1">
              <Command.Empty className="px-3 py-2 text-sm text-gray-500">{emptyText}</Command.Empty>
              {filtered.map((opt) => (
                <Command.Item
                  key={opt.value}
                  value={opt.label}
  ...
  // Input trigger mode
  const { inputPlaceholder = 'Type to search…', inputClassName } = props as InputModeProps
