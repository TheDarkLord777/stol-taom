...
export function DatePicker({
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        ...
        >
          <span className="truncate">
            {date ? format(date, formatString) : <span className="text-gray-500">{placeholder}</span>}
          </span>
      ...
      <Popover.Content sideOffset={8} className={popoverClassName || 'z-50 rounded-md border border-gray-200 bg-white p-2 shadow-lg'}>
        <Calendar
          mode="single"
          selected={date}
          onSelect={(d) => {
...
export function DateRangePicker({
  ...
  const [open, setOpen] = React.useState(false)
  function label() {
    if (range?.from && range?.to) return `${format(range.from, formatString)} — ${format(range.to, formatString)}`
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        ...
        >
          <span className="truncate">{label() || <span className="text-gray-500">{placeholder}</span>}</span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
            <line x1="16" y1="2" x2="16" y2="6"></line>
            <line x1="8" y1="2" x2="8" y2="6"></line>
      ...
      <Popover.Content sideOffset={8} className={popoverClassName || 'z-50 rounded-md border border-gray-200 bg-white p-2 shadow-lg'}>
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="ghost" size="sm" onClick={() => onRangeChange?.(undefined)}>Clear</Button>
