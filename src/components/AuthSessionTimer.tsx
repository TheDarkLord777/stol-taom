...
export default function AuthSessionTimer({
  ...
  const fetchDebug = React.useCallback(async () => {
    try {
      const r = await fetch("/api/auth/debug", {
        cache: "no-store",
  ...
  const accessTone: "ok" | "warn" | "bad" | "muted" =
    aTtl == null ? "muted" : aTtl <= 0 ? "bad" : aTtl <= 10 ? "warn" : "ok";
  const refreshTone: "ok" | "warn" | "bad" | "muted" =
    rTtl == null ? "muted" : rTtl <= 0 ? "bad" : rTtl <= 60 ? "warn" : "ok";

  return (
    <div
      className={`rounded-lg border border-white/10 bg-white/5 p-3 ${className}`}
    >
      <div className="mb-2 text-xs font-medium text-gray-300">
        Jonli sessiya taymeri
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <Pill
          label="Access"
          value={aTtl != null ? `${Math.max(0, Math.floor(aTtl))}s` : "—"}
          tone={accessTone}
        />
        <Pill
          label="Refresh"
          value={rTtl != null ? `${Math.max(0, Math.floor(rTtl))}s` : "—"}
          tone={refreshTone}
        />
        {data?.enableRedis != null && (
          <Pill
            label="Redis"
            value={
              data.enableRedis ? (data.connected ? "connected" : "down") : "off"
            }
