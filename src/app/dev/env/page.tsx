...
export default function DevEnvPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/dev/env/list')
      ...
      .then((d) => {
        ...
        const list = fs.filter((f: string): f is KnownFile => (knownFiles as readonly string[]).includes(f))
        setFiles(list)
        if (list.length) setFile((prev) => (list.includes(prev) ? prev : list[0]))
      })
      .catch(() => {})
    return () => {
        mounted = false
  ...
  const isBool = (v: string | undefined) => v !== undefined && (v.toLowerCase() === 'true' || v.toLowerCase() === 'false')

  const updateValue = (k: string, v: string | boolean) => {
    setValues((prev) => ({ ...prev, [k]: typeof v === 'boolean' ? (v ? 'true' : 'false') : String(v) }))
  }
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            .map((k) => {
              ...
              return (
                <div key={k} className="flex items-center justify-between gap-4">
                  <div className="w-1/3 font-mono text-sm truncate" title={k}>{k}</div>
      ...
      <div className="text-xs text-gray-500">
        Tip: Next.js reads envs at process start. For production (.env.production), restart may be required. Prisma CLI uses .env.
      </div>
    </div>
  )
}
