...
export default function IngredientsAdminClient({ initialItems, initialRestaurants }: { initialItems: Item[]; initialRestaurants: Restaurant[] }) {
    ...
    const addIngredient = () => {
        setLocalIngredients((prev) => [...prev, { id: `new-${Date.now()}`, name: "", mandatory: false }]);
    };
    const removeIngredient = (id: string) => {
        setLocalIngredients((prev) => prev.filter((i) => i.id !== id));
    };
    const updateIngredientName = (id: string, name: string) => {
        setLocalIngredients((prev) => prev.map((i) => (i.id === id ? { ...i, name } : i)));
    };
    const toggleIngredientMandatory = (id: string) => {
        setLocalIngredients((prev) => prev.map((i) => (i.id === id ? { ...i, mandatory: !i.mandatory } : i)));
    };

    const toggleRestaurant = (rid: string) => {
        setLocalRestaurantIds((prev) => (prev.includes(rid) ? prev.filter((r) => r !== rid) : [...prev, rid]));
    };
    ...
    const saveAll = async () => {
        ...
        try {
            ...
            const ingPayload = localIngredients.map((i) => ({ name: i.name, mandatory: i.mandatory }));
            ...
            setItems((prev) => prev.map((it) => (it.id === editing.id ? { ...it, ingredients: (data1.ingredients ?? []).map((x: any) => ({ id: String(x.id), name: x.name, mandatory: Boolean(x.mandatory) })), restaurantIds: (data2.assigned ?? []).map((a: any) => a.restaurantId) } : it)));
            ...
            <div className="overflow-auto border rounded">
                <table className="min-w-full divide-y">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Menu item</th>
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Ingredients</th>
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Restaurants</th>
                                ...
                                <td className="px-4 py-3 align-top">
                                    <div className="flex flex-wrap gap-2">
                                        {(it.ingredients ?? []).map((ing) => (
                                            <span key={ing.id} className="rounded border px-2 py-1 text-sm">
                                                {ing.name} {ing.mandatory ? <span className="text-xs text-red-500">(mandatory)</span> : null}
                                            </span>
                                        ))}
                                    </div>
                                </td>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {(it.restaurantIds ?? []).length > 0 ? (
                                            it.restaurantIds.map((rid) => {
                                                ...
                                                return (
                                                    <span key={rid} className="rounded border px-2 py-1 text-sm">
                                                        {r ? r.name : rid}
                                                    </span>
                                                );
                                            })
                    ...
                    <div className="relative w-full max-w-3xl rounded bg-white p-6 shadow-lg">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <button className="text-sm text-gray-500" onClick={closeEdit} disabled={saving}>Cancel</button>
                                <button className="rounded bg-green-600 text-white px-3 py-1 text-sm" onClick={saveAll} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button>
                            </div>
                        </div>
                        ...
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div>
                                        <button className="rounded bg-blue-600 text-white px-3 py-1 text-sm" onClick={addIngredient}>Add ingredient</button>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-2 max-h-64 overflow-auto">
                                    {restaurants.map((r) => (
                                        <label key={r.id} className="flex items-center gap-2">
                                            <input type="checkbox" checked={localRestaurantIds.includes(r.id)} onChange={() => toggleRestaurant(r.id)} />
                                            <span>{r.name}</span>
                                        </label>
                                    ))}
                                </div>
