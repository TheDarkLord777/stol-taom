            ...
            <main className="p-6 mx-auto max-w-4xl">
                <div className="flex items-center gap-4">
                    {restaurant.logoUrl ? (
                        ...
                        <img src={restaurant.logoUrl} alt={restaurant.name} className="h-24 w-24 rounded object-cover" />
                    ) : (
                ...
                <section className="mt-6">
                    ...
                    {menu.length === 0 ? (
                        <div className="text-sm text-gray-500 mt-2">No menu items found for this restaurant.</div>
                    ) : (
                        <ul className="mt-3 space-y-3">
                            {menu.map((m) => (
                                <li key={m.id} className="p-3 border rounded flex items-center justify-between">
                                    ...
                                    <div>
                                        <a href={`/menu?restaurant=${encodeURIComponent(String(id))}`} className="text-blue-600 text-sm">View on menu</a>
                                    </div>
                                </li>
                            ))}
                        </ul>
