...
export default function RestaurantManagementPage() {
    ...
    const [status, setStatus] = React.useState<'loading' | 'not-auth' | 'checking' | 'allowed' | 'denied'>('loading');
    ...
    React.useEffect(() => {
        ...
        async function init() {
            ...
            try {
                const direct = await apiFetch(`/api/management/orders?restaurantId=${encodeURIComponent(restaurantId)}`);
                if (!mounted) return;
                if (direct.ok) {
            ...
            try {
                const res = await fetch(`/api/management/check?restaurantId=${encodeURIComponent(restaurantId)}`, { credentials: 'same-origin' });
                if (!mounted) return;
                const j = await res.json().catch(() => null);
                if (j?.allowed) {
                    setStatus('allowed');
                    try {
                        const or = await apiFetch(`/api/management/orders?restaurantId=${encodeURIComponent(restaurantId)}`);
                        if (!mounted) return;
                        if (!or.ok) {
    ...
    async function doLogin(e?: React.FormEvent) {
        ...
        setMessage(null);
        try {
            const res = await fetch('/api/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'same-origin', body: JSON.stringify({ phone, password }) });
            const j = await res.json().catch(() => null);
            if (!res.ok) return setMessage(j?.error || `Login failed: ${res.status}`);
            // after successful login, re-check manager access
            const check = await fetch(`/api/management/check?restaurantId=${encodeURIComponent(restaurantId)}`, { credentials: 'same-origin' });
    ...
    function ClientAccordion({ label, items }: { label: string; items: any[] }) {
        ...
        return (
            <div className="border rounded overflow-hidden">
                <button type="button" onClick={() => setOpen((s) => !s)} className="w-full flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900">
                    <div className="text-left">
                        <div className="font-medium text-gray-900 dark:text-gray-100">{label}</div>
                        <div className="text-xs text-gray-600 dark:text-gray-400">{items.length} item{items.length !== 1 ? 's' : ''} — {latest}</div>
                    </div>
                        ...
                        {items.map((o) => (
                            <div key={o.id} className="p-2 border rounded">
                                <div className="flex justify-between">
                                    <div className="font-medium text-gray-900 dark:text-gray-100">{o.type === 'order' ? `Order — ${o.status ?? '—'}` : `Reservation — Party: ${o.partySize ?? '—'}`}</div>
                                    <div className="text-xs text-gray-600 dark:text-gray-400">{o.createdAt ? new Date(o.createdAt).toLocaleString() : ''}</div>
                                </div>
                                {o.type === 'order' ? (
                                    <div className="mt-2 text-sm text-gray-700 dark:text-gray-200">
                                        <div className="text-sm text-gray-600 dark:text-gray-300">Payment: {o.paymentMethod ?? '—'}</div>
                                        <div className="mt-2 space-y-1">
                                            {Array.isArray(o.items) && o.items.length > 0 ? o.items.map((it: any) => (
                                                <div key={it.id} className="flex justify-between text-sm text-gray-800 dark:text-gray-200">
                                                    <div>{it.name} x{it.quantity}</div>
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">{it.price != null ? `${it.price}` : ''}</div>
                                                </div>
                                    ...
                                    <>
                                        <div className="mt-2 text-sm text-gray-700 dark:text-gray-200">From: {o.fromDate ? new Date(o.fromDate).toLocaleString() : '—'}{o.toDate ? ` — ${new Date(o.toDate).toLocaleString()}` : ''}</div>
                                        {o.note ? <div className="mt-2 text-sm text-gray-700 dark:text-gray-200">Note: {o.note}</div> : null}
                                    </>
                                )}
                            </div>
                        ))}
    ...
    return (
        <main className="p-6 mx-auto max-w-3xl">
            ...
            {status === 'loading' || status === 'checking' ? <div>Checking access...</div> : null}

            {status === 'not-auth' && (
                <div>
                    <p className="mb-3">You must sign in as a manager for this restaurant.</p>
                    <form onSubmit={doLogin} className="space-y-2">
                        ...
                        <input placeholder="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded border px-2 py-1" />
                        <div className="flex gap-2">
                            <button className="rounded bg-emerald-500 text-white px-3 py-1" type="submit">Sign in</button>
                            <button type="button" className="rounded bg-gray-200 px-3 py-1" onClick={() => router.push('/management')}>Back</button>
                        </div>
                        {message ? <div className="text-sm text-red-600 mt-2">{message}</div> : null}
                    </form>
                </div>
            )}

            {status === 'denied' && (
                <div className="text-red-600">Access denied. You are signed in but not a manager for this restaurant.</div>
            )}

            {status === 'allowed' && (
                <div>
                    <h2 className="text-lg font-semibold mb-2">Manager Dashboard</h2>
                    <p className="text-sm text-gray-600 mb-4">You can now manage this restaurant (placeholder UI).</p>
                    <div className="space-y-2">
                        <div className="flex gap-2 items-start">
                            <button className="rounded bg-sky-500 text-white px-3 py-1" onClick={async () => {
                                setMessage(null);
                                setStatus('checking');
                                try {
                                    const res = await apiFetch(`/api/management/orders?restaurantId=${encodeURIComponent(restaurantId)}`);
                                    if (!res.ok) {
                                        const j = await res.json().catch(() => null);
                            ...
                            <div>
                                <small className="text-xs text-gray-500">Click to load reservations (open orders)</small>
                                {message ? <div className="text-sm text-red-600">{message}</div> : null}
                            </div>
                        </div>
                        <button className="rounded bg-emerald-600 text-white px-3 py-1">Edit Menu</button>
                        <button type="button" className="rounded bg-gray-200 px-3 py-1 text-sm" onClick={() => setShowRaw((s) => !s)}>{showRaw ? 'Hide raw' : 'Show raw'}</button>
                    </div>
                </div>
            )}
            {showRaw && (
                <div className="mt-4 p-3 border rounded bg-white dark:bg-gray-900">
                    ...
                    <pre className="text-xs overflow-auto max-h-64">{JSON.stringify(openOrders, null, 2)}</pre>
                </div>
            )}
