...
export default function RedisDebugger() {
    ...
    if (loading) return <div className="text-sm text-gray-400">Loading Redis...</div>;
        ...
        <div>
            ...
            <div className="space-y-2">
                {keys.length === 0 ? (
                    ...
                    keys.map((k) => (
                        <div key={k.key} className="flex items-center justify-between rounded border p-2">
                            <div className="flex flex-col">
                                <div className="font-mono text-sm">{k.key}</div>
                                <div className="text-xs text-gray-400">TTL: {k.ttlSec}</div>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => doRefresh(k.key)} className="text-sm text-blue-400">Refresh</button>
                                <button onClick={() => doDelete(k.key)} className="text-sm text-red-400">Delete</button>
                            </div>
                        </div>
                    ))
                )}
            ...
            <div className="mt-3">
                <button onClick={load} className="text-sm text-gray-300">Refresh list</button>
            </div>
        </div>
    );
}
