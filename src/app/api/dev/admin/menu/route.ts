...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { id, name, slug, logoUrl, restaurantId, price } = body as { id?: string; name: string; slug?: string; logoUrl?: string; restaurantId?: string; price?: string };
        const normalize = (s: string) => s.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-_]/g, '').slice(0, 200);
        ...
        if (id) {
            ...
            const createData = { name, slug: slugNormalized ?? normalize(name), logoUrl: logoUrl ?? null };
        ...
        if (restaurantId) {
            try {
                await prisma.menuItemOnRestaurant.upsert({
                    where: { menuItemId_restaurantId: { menuItemId: row.id, restaurantId } },
                    update: {},
        ...
        return NextResponse.json({ menuItem: { id: row.id, name: row.name, slug: row.slug, logoUrl: row.logoUrl ?? undefined } });
    } catch (err) {
