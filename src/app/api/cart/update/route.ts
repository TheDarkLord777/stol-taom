...
export async function PATCH(req: NextRequest) {
    try {
        ...
        if (!body?.id || typeof body.quantity !== "number") return NextResponse.json({ error: "Invalid body" }, { status: 400 });
        ...
        const updated = await prisma.cartItem.update({ where: { id: body.id }, data: { quantity: body.quantity } });
