...
export async function GET(req: NextRequest) {
    try {
            ...
            try {
                ...
                if (cached) {
                    ...
                    if (cookieResponse) {
                        ...
                        for (const sc of setCookies) resJson.headers.append("set-cookie", sc);
                    }
                    return resJson;
                }
        ...
        try {
            // Long-lived cache (weekly). Use /api/orders/invalidate or refresh=1 to update on demand.
            if (r && cacheKey) await r.set(cacheKey, JSON.stringify(body), "EX", 60 * 60 * 24 * 7);
        } catch { /* ignore */ }
        if (cookieResponse) {
