  ...
  try {
    ...
    if (!requestId || !code || !newPassword) {
      return NextResponse.json(
        { error: "requestId, code va yangi parol talab qilinadi" },
        { status: 400 },
      );
    }
    ...
    if (!rec || !rec.userId) {
      return NextResponse.json(
        { error: "So'rov topilmadi yoki muddati o'tgan" },
        { status: 404 },
      );
    }
    ...
    if (verifyRes) {
      vStatusRaw =
        (verifyRes.result as Record<string, unknown> | undefined)?.status ??
        (verifyRes.status as unknown) ??
        undefined;
    }
    ...
    const isVerified =
      (verifyRes && verifyRes.verified === true) === true ||
    ...
    if (!isVerified) {
      ...
      if (EXPIRED_STATES.has(String(vStatus)))
        return NextResponse.json({ error: "Kod eskirgan" }, { status: 410 });
      if (PENDING_STATES.has(String(vStatus)))
        return NextResponse.json(
          { error: "Kod hali tasdiqlanmagan" },
          { status: 409 },
        );
