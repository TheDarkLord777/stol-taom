...
export async function POST(req: NextRequest) {
    try {
        ...
        if (cooldown > 0) {
            return NextResponse.json({ error: "Iltimos bir oz kuting", retryAfter: cooldown }, { status: 429 });
        }
        ...
        if (!user) {
            ...
            if (devAdminEnabled) return NextResponse.json({ ok: true, registered: false });
            return NextResponse.json({ ok: true });
        }

        // Send verification via gateway
        const sendRes = (await sendVerificationMessage(phone)) as Record<string, unknown> | undefined;
        const requestId = (sendRes && (sendRes.result as Record<string, unknown> | undefined)?.request_id) || (sendRes && (sendRes as any).request_id) || undefined;
        const reqIdStr = String(requestId ?? `f_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);

        await forgotRepo.set({ requestId: reqIdStr, phone, userId: user.id, createdAt: Date.now() });
        await forgotRepo.setCooldown(phone, 30);
