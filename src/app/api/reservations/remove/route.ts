...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const resv = await prisma.reservation.findUnique({ where: { id: body.id } });
    if (!resv || resv.userId !== user.id) return NextResponse.json({ error: "Reservation not found" }, { status: 404 });
