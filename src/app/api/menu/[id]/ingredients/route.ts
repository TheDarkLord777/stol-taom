...
export async function GET(req: NextRequest) {
    ...
    try {
            ...
            try {
                ...
                if (dbUrl) {
                    try {
                        ...
                        console.info("[debug] DATABASE_HOST=", parsed.hostname, "PORT=", parsed.port || '5432');
                    } catch {
        ...
        const refreshAheadMs = Number(process.env.INGREDIENT_CACHE_REFRESH_AHEAD_MS ?? 2 * 60 * 1000);

        // helper: refresh the cached ingredients for id into redis
        async function refreshIngredientsToRedis(rClient: any, prismaClient: any, menuId: string, ttl: number) {
            try {
                const items = await prismaClient.ingredient.findMany({ where: { menuItemId: menuId }, orderBy: { createdAt: "asc" } });
        ...
        if (r) {
            try {
                ...
                if (cached) {
                    ...
                    return NextResponse.json({ ingredients: parsed }, { headers: { "X-Cache": "HIT" } });
                }
            } catch (err) {
                // eslint-disable-next-line no-console
        ...
        const items = await prisma.ingredient.findMany({ where: { menuItemId: id }, orderBy: { createdAt: "asc" } });
        if (r) {
            try {
                const verRaw = await r.get(`menu:ingredients:version:${id}`);
                const ver = verRaw ? Number(verRaw) : 0;
        ...
        return NextResponse.json({ ingredients: items }, { headers: { "X-Cache": "MISS" } });
    } catch (e: unknown) {
    ...
    try {
        ...
        if (r) {
            try {
                ...
                // eslint-disable-next-line no-console
                console.info("[menuRepo:cache] bumped ingredients version and wrote", redisKey);
            } catch (err) {
                // eslint-disable-next-line no-console
                console.warn("[menuRepo:cache] redis version bump error ingredients", err);
            }
        }

        return NextResponse.json({ ingredients: updated, cacheInvalidated: true }, { headers: { "X-Cache": "INVALIDATED" } });
    } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
