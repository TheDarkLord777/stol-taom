      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-12">
        <div className="w-full max-w-md space-y-6">
          <div className="flex justify-center">
            <h1 className="bg-[#7B9EFF] px-12 py-3 text-3xl font-bold text-white rounded-lg shadow-lg">Kod tasdiqlash</h1>
          </div>
          <div className="space-y-2 text-white text-center">
            <p>{phone ? `${phone} raqamiga yuborilgan 6 xonali kodni kiriting` : '6 xonali kodni kiriting'}</p>
          </div>
          <div className="flex justify-center">
            <Input
              inputMode="numeric"
          ...
          <div className="flex justify-center pt-2">
            <Button onClick={onSubmit} disabled={loading || cooldown > 0} className="h-16 w-full max-w-xs bg-[#C8FF00] hover:bg-[#B8EF00] text-black text-2xl font-bold shadow-xl rounded-lg">
              {cooldown > 0 ? `Qayta urinish: ${cooldown}s` : loading ? 'Yuborilmoqda...' : 'Tasdiqlash'}
            </Button>
          </div>
        </div>
      </div>
